//! The open document: its state, and the plain logic that acts on it.
//!
//! Everything here is callable without a running Tauri app or a native file
//! dialog, which is what makes it testable. The thin command wrappers that
//! expose it to the frontend live in `commands.rs`.
...
pub(crate) fn reset(state: &DocumentState) {
...
pub(crate) fn open_at(state: &DocumentState, path: PathBuf) -> Result<OpenedDocument, String> {
...
pub(crate) fn save_at(state: &DocumentState, path: PathBuf, text: String) -> Result<String, String> {
...
pub(crate) fn current_path(state: &DocumentState) -> Option<PathBuf> {
    state.inner.lock().unwrap().path.clone()
}

pub(crate) fn compile_at(state: &DocumentState, text: &str) -> CompileResult {
    let mut inner = state.inner.lock().unwrap();
...
pub(crate) fn complete_at(state: &DocumentState, text: &str, cursor: usize) -> Completions {
    let mut inner = state.inner.lock().unwrap();
    let Inner { session, document, .. } = &mut *inner;
    session.completions(text, cursor, document.as_ref())
}

pub(crate) fn jump_at(state: &DocumentState, page: usize, x_pt: f64, y_pt: f64) -> Option<usize> {
...
pub(crate) fn preview_position_at(state: &DocumentState, cursor: usize) -> Option<PreviewPosition> {
    let inner = state.inner.lock().unwrap();
    let document = inner.document.as_ref()?;
    inner
        .session
...
pub(crate) fn render_page_at(state: &DocumentState, index: usize, pixel_per_pt: f32) -> Result<RenderedPage, String> {
